...
如果你通过[HTTP API](./rest_api)来使用数据库，
...
想了解更多有关压缩算法的内容可以看[压缩算法详情](./design#压缩算法)。
...
如果 VALUES 列表需要表达式，请使用[INSERT SELECT](#insert-select)语法。
...
关于 SHOW QUERIES 语句的详细信息，可以在[系统表 QUERIES](#queries)查看。
...
注意，该视图目前只会在通过[lineprotocol](./rest_api#接口列表)/[prometheus remote write](./ecosystem#remote-write) 接口写入成功时创建。
...
KILL [QUERY] query_id;
```
先通过 [`SHOW QUERIES`](#show-queries) 获取 `query_id`。
