...
1. 扩展性，时间序列膨胀（理论无上限）支持横/纵向扩展。
2. 计算存储分离，计算节点和存储节点，可以独立扩缩容。
3. 存储性能和成本，高性能 io，支持云盘和对象存储进行分级存储。
...
cnosdb-isihpo 的分片规则使用基于 time-range 的分区规则
...
![数据分割目录存储](./ource/_static/img/write.jpg)
...
![数据分割目录存储](./source/_static/img/read.jpg)
