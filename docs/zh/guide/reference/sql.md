...
想了解更多有关压缩算法的内容可以看[压缩算法详情](../design/implementation.md)。
...
CnosDB支持两种数据写入的方法，一种是使用`INSERT INTO`语句，另一种是使用HTTP API的[write](../development/rest_api.md)接口，写入Line Protocol格式数据。
...
CnosDB 要求插入的数据列必须要有时间戳，且VALUES列表必须为[常量](./sql.md#常量)，
如果有列没有被选中，那么值为`NULL`。
