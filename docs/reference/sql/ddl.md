...
> 数据过期策略请参考 [分片规则](https://www.baidu.com)
...
不同的数据类型可以指定不同的压缩算法，支持列表请参考 [压缩算法](压缩算法)
...
如果需要对创建之后的表进行其他操作，请参考 [`INSERT`](#insert)、 [`ALTER TABLE`](#alter-table)、[`DROP TABLE`](#drop-table)。
...
> `ALTER DATABASE` 可以修改数据库中设定的所有参数（不包含 `PRECISION`），你可以通过 [DESCRIBE DATABASE](dml#describe-database) 命令查询当前数据库参数设置。
