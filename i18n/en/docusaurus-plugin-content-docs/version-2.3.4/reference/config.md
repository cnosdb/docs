...
| `using_raft_replication` Raft protocol is used for replica replication. Note: it is not stable at present, so it is not recommended for online use. | `128M`                                                 | `COPY INTO` Export triggers the memory size of the disk.                              |
...
| meta_service_port                           | `8906`           | Used to listen for data written by [Vector](https://vector.dev/). |
