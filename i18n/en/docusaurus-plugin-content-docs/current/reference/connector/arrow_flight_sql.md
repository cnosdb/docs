...
Each Endpoint represents a certain location that contains a subset of response data.

A FlightEndpoint contains a list of server addresses,
a Ticket, and a binary Token used by the server to identify request data.
FlightEndPoint has no defined order, if the dataset is sorted,
Only data will be returned in one FlightEndPoint.
