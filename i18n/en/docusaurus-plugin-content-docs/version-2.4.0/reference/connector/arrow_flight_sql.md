...
:::info Below is a description of how different clients use：
...
* #### 设置属性并执行SQL
  ...
  ```@tab ODBC#ODBC目前仅支持x86_64架构的系统，linux仅支持centos和redhat系列发行版。For more information about Arrow Flight SQL ODBC, see[Dremio文档](https://docs.dremio.com/software/drivers/arrow-flight-sql-odbc-driver/).The following steps are based on Centos7.* ### Install ODBC Manager
...
- #### Create Flight SQL Client
  ...
  addr specifies the address for the CnosDB configuration entry `flight_rpc_listen_addr`

- #### Set the connection voucher and get the proven context
...
- #### Execute SQL in proven context and get Flash Info
...
- #### Get Reader from FlowtInfo

  ```go
  // CnosDb only implements one EndPoint
  rdr, err := cl.DoGet(ctx, info.GetEndpoint()[0].Ticket)
  if err !=nil LO
    fmt. rint(err)
    fmt.Println(35)
...
Code is running in an asynchronous environment.
